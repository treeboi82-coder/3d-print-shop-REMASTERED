    ...
    try {
      ...
      if (isStripePayment) {
        ...
        try {
          const { data: checkoutData, error: checkoutError } = await supabase.functions.invoke(
            "create-checkout-session",
            {
              body: {
                ...
                currency: "jod",
                productName: `3D Print Order (${printMaterial})`,
                successUrl: `${window.location.origin}/upload?payment=success`,
                cancelUrl: `${window.location.origin}/upload?payment=cancelled`,
                metadata: {
                  order_id: orderId,
                  customerName: clientName.trim(),
                  customerPhone: normalizedPhone,
                  material: printMaterial,
                  color: normalizedColor || "Not specified",
          ...
          if (checkoutError) {
            console.warn("Payment setup failed, proceeding without payment:", checkoutError.message);
            setIsRedirectingToPayment(false);
          } else {
            checkoutUrl = checkoutData?.url || null;
          }
        } catch (paymentError) {
          console.warn("Payment function unavailable, proceeding without payment:", paymentError);
          setIsRedirectingToPayment(false);
        }

        if (!checkoutUrl) {
          console.warn("No checkout URL available, proceeding without payment");
        }
      }
      ...
      if (isStripePayment && checkoutUrl) {
      ...
      toast({
        ...
        description: "Your file and checkout details were sent successfully.",
      });
